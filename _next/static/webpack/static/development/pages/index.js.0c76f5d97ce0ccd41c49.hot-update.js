...
var glowSymbols = function glowSymbols(glowChunk, tail, index, p5) {
  if (index - glowChunk > 0) {
    for (var i = 0; i < glowChunk; i++) {
      tail[index - i].shouldGlow = true; //tail[index - i].symbol = getRandomSymbol();
...
var Symbol = /*#__PURE__*/function () {
  ...
  Object(_babel_runtime_helpers_esm_createClass__WEBPACK_IMPORTED_MODULE_1__["default"])(Symbol, [{
    ...
    value: function initSymbol(p5, positionX) {
      ...
      this.rainSpeed = getRandomNumber(30, 20);
    }
  }, {
    key: "update",
    value: function update(p5) {
      ...
      var changeSymbol = Math.random() > 0.98; //Math.random() > 0.95;
      // if (changeSymbol) {
      //   this.symbol = getRandomSymbol();
      // }
      // if (
      ...
      var _loop = function _loop(i) {
        ...
        if (p5.frameCount % 2 == 0) _this.tail[i].shadeOffset -= 2;

        if (changeSymbol) {
          setTimeout(function () {
            glowSymbols(getRandomNumber(2, 1), _this.tail, i, p5); // this.tail[i].shouldGlow = true;

            _this.tail[i].symbol = getRandomSymbol(); //   this.tail[i].shadeOffset = getRandomNumber(255, 80);
          }, _this.tail.length * 100 - i * _this.rainSpeed);
        }

        if (_this.tail[i].shouldGlow) {
          // i < 1 ||
          p5.fill(120, 255, 120, _this.tail[i].shadeOffset * 1.2);
...
//# sourceMappingURL=index.js.0c76f5d97ce0ccd41c49.hot-update.js.map